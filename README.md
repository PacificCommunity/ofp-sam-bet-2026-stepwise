...
| S02 | Region 5 index asymptotic selectivity | `S02-F33Asymptotic-MIX015` | `S01-SelectivityStability-MIX015` | Retain extraction-only sharing and independent indices, and replace only the F33 four-node spline with an independent logistic selectivity. | S01 inputs and controls; Kflow uses tuna-flow v2.6 with the corrected pre-mixing reporting-rate calculation. | Post-selection sensitivity; retain only if fit is maintained and stability improves. |
