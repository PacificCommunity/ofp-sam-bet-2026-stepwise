...
# This runner starts from bet.ini for every scenario and changes only the
# phase at which MFCL converts the freshly fitted standard recruitment state
# to OPR coefficients.  The complete switch block is repeated whenever OPR is
# activated; changing pf155 alone would leave incompatible standard-recruitment
# controls active.
switch_phase=${OPR_SWITCH_PHASE:-3}
opr_year=${OPR_YEAR:-72}
opr_season=${OPR_SEASON:-1}
opr_region=${OPR_REGION:-50}
opr_season_region=${OPR_SEASON_REGION:-50}
opr_end_years=${OPR_END_YEARS:-2}
opr_compatibility_year=${OPR_COMPATIBILITY_YEAR:-0}
terminal_penalty=${OPR_TERMINAL_PENALTY:-100}

case "$switch_phase" in
  3|8|10) ;;
  *)
    echo "OPR_SWITCH_PHASE must be one of 3, 8, or 10 for this runner." >&2
    exit 2
    ;;
esac
for value in "$opr_year" "$opr_season" "$opr_region" "$opr_season_region" \
  "$opr_end_years" "$opr_compatibility_year" "$terminal_penalty"; do
  case "$value" in
    ''|*[!0-9]*)
      echo "OPR settings and OPR_TERMINAL_PENALTY must be non-negative integers." >&2
      exit 2
      ;;
  esac
done
...
opr_switch_block() {
  cat <<EOF
# Complete standard-recruitment -> OPR conversion block.
  1 149 0
  1 398 0
  1 400 0
  2 177 0
...
if [ "$switch_phase" -eq 3 ]; then
  phase3_block="$(opr_switch_block)
  1 1 500"
else
  phase3_block='  2 70 1
  2 71 1
...
if [ "$switch_phase" -le 3 ]; then
  phase5_distribution_block='  -100000 1 0
  -100000 2 0
  -100000 3 0
  -100000 4 0
  -100000 5 0'
else
  phase5_distribution_block='  -100000 1 1
  -100000 2 1
  -100000 3 1
  -100000 4 1
  -100000 5 1'
...
phase8_switch_block=''
if [ "$switch_phase" -eq 8 ]; then
  phase8_switch_block="$(opr_switch_block)"
fi
phase10_switch_block=''
if [ "$switch_phase" -eq 10 ]; then
  phase10_switch_block="$(opr_switch_block)"
fi

echo "OPR phase-placement scenario: switch at Phase ${switch_phase}; settings ${opr_year}-${opr_season}-${opr_region}-${opr_season_region}; E${opr_end_years}; terminal penalty ${terminal_penalty}"
...
$(opr_switch_block)
  1 397 $terminal_penalty  # terminal-recruitment penalty; native weight is flag/10
...
# All phase-placement variants report a common final filename while retaining
# the numbered phase PARs for native auditability.
cp 11.par final.par
