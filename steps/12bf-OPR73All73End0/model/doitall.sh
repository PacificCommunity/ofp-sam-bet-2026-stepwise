...
# 12bf OPR: all effects saturated with no multi-year endpoint ties.
...
# The ordinary recruitment-deviation terminal controls below are disabled before OPR is activated.
  ...
  1 155 73  # annual OPR coefficient count (polynomial degree = count - 1)
  1 217 73   # seasonal OPR coefficient count (polynomial degree = count - 1)
  1 216 73  # regional OPR coefficient count (polynomial degree = count - 1)
  1 218 73  # season-by-region OPR coefficient count (polynomial degree = count - 1)
  1 202 0   # year effect: no multi-year terminal tie (MFCL normalizes the zero window to one endpoint)
  1 203 0   # year effect: all non-constant terms are flat in its end window
  1 210 -1   # region effect: no multi-year terminal tie (MFCL normalizes this to one endpoint)
  1 211 0   # region effect: all non-constant terms are flat in its end window
  1 212 -1   # season effect: no multi-year terminal tie (MFCL normalizes this to one endpoint)
  1 213 0   # season effect: all non-constant terms are flat in its end window
  1 214 -1   # season-by-region effect: no multi-year terminal tie (MFCL normalizes this to one endpoint)
  1 215 0   # season-by-region effect: all non-constant terms are flat in its end window
