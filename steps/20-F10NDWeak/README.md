# 20 F10 weak non-decreasing penalty

Add only the deterministic Job 19325 treatment to the Step 19 model: F10
LL.ALL.5 fish flags 16=1 and 56=10000. The model starts from the ordinary
`-makepar` output and does not use jitter, perturbation, or the promoted
seed-23 solution.
...
| Field | Value |
| --- | --- |
| Step folder | `steps/20-F10NDWeak/model` |
...
| # | Change |
| --- | --- |
| 1 | Add the F10 LL.ALL.5 weak non-decreasing penalty with fish flags 16=1 and 56=10000. |
...
| 6 | Scientific parent: '19-DMG8Nmax25'. |
...
| `Step-specific change` | Add only F10 fish flags 16=1 and 56=10000, matching the deterministic Job 19325 fit. | All Step 19 data, DM, tag, recruitment, movement, mortality, and selectivity controls. |
...
|  8 | F10 retains five estimated cubic-spline nodes; flag 16=1 activates the non-decreasing penalty and flag 56=10000 sets its weak weight. |
|  9 | The folder is generated independently from source inputs; its scientific parent is not a runtime dependency. |
| 10 | No jitter, parameter perturbation, promoted seed, OPR, or length-bin selectivity controls are used. |
