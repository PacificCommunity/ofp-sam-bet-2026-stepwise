...
# Selectivity groups for the robustness sensitivity.
# Each regional index shares selectivity only with its matched extraction fishery:
# F29/F2 (R1), F30/F4 (R2), F31/F7 (R3), and F32/F8 (R4).
# F33 (R5) remains independent. Catchability groups are controlled separately
# by flag 99 and are not shared by these flag-24 settings.
  ...
  -30 24 4   # Index R2 shares selectivity with F4 LL.ALL.2
  -31 24 7   # Index R3 shares selectivity with F7 LL.WEST.3
  -32 24 8   # Index R4 shares selectivity with F8 LL.EAST.3
  -33 24 29  # Index R5 retains an independent selectivity (next contiguous group)
...
# Set Lorenzen M
  ...
  1 121 0    # keep the Lorenzen intercept fixed at the bet.ini start (-2.5) during model construction
...
# MFCL 2.2.2.0 growth variance fix
  ...
  -1 61 4   # F1 four cubic-spline nodes in the robustness sensitivity
  -3 61 4   # F3 four cubic-spline nodes in the robustness sensitivity
  -5 61 4   # F5 four cubic-spline nodes in the robustness sensitivity
...
# STAGED MFCL RUN 5: retain the sensitivity's regional index/extraction
# selectivity groups after assigning each index an independent flag-99
# catchability/likelihood group above.
  -29 24 2   # Index R1 retains the F2 LL.EAST.1 selectivity
  -30 24 4   # Index R2 retains the F4 LL.ALL.2 selectivity
  -31 24 7   # Index R3 retains the F7 LL.WEST.3 selectivity
  -32 24 8   # Index R4 retains the F8 LL.EAST.3 selectivity
  -33 24 29  # Index R5 retains an independent selectivity (next contiguous group)
PHASE5
...
$program_path bet.frq 09.par 10.par -file - <<PHASE10
  ...
  1 121 1    # estimate the Lorenzen natural-mortality intercept from the -2.5 start; retain the fixed length slope
PHASE10
...
$program_path bet.frq 10.par 11.par -file - <<PHASE11
  1 1 5000
  1 50 $phase10_11_convergence  # convergence criterion; stepwise default -4 (MGC target 1e-4)
  1 121 1   # continue estimating the Lorenzen natural-mortality intercept
