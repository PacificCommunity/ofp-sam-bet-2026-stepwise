# 16 Fleet-specific selectivity configuration

Reconfigure fleet-specific selectivity for the revised fishery structure, remove six superseded legacy controls, and set flag 16 to 0 for all 14 applicable fisheries so all weighting comparisons use the selected Job 14363 setting.
...
| # | Change |
| --- | --- |
| 1 | Reconfigure fleet-specific selectivity for the revised fishery structure, remove six superseded legacy controls, and set flag 16 to 0 for all 14 applicable fisheries so all weighting comparisons use the selected Job 14363 setting. |
| 2 | Scientific rationale: represent fleet-specific size availability without imposing unnecessary older-age shape constraints before comparing composition weighting. |
...
| 4 | The Step 15b parent has 15 active flag-16 penalties. The revised structure changes the applicable fleet set: superseded F20/F28 controls are removed and F15 is introduced. |
| 5 | For the resulting F12, F13, F15-F19, and F21-F27 set, flag 16 is 0 (form penalty off); fleet-specific terminal ages, spline-node counts, and youngest-age tails are retained. |
...
| `Step-specific change` | Reconfigure fleet-specific selectivity for the revised fishery structure, remove six superseded legacy controls, and set flag 16 to 0 for all 14 applicable fisheries so all weighting comparisons use the selected Job 14363 setting. | All previously selected controls; no OPR or length-bin selectivity. |
