# 23d: K=0.2 mean over KS with one common tag tau

This sensitivity is an exact clone of `22b-MIX020-MeanOverKS` except for the
tag-recapture tau controls added to `doitall.sh` from Phase 10 onward.

| Item | Setting |
| --- | --- |
| Scientific parent | `22b-MIX020-MeanOverKS` |
| Mixing periods | SC22-IP10 K=0.2 mean over KS; unchanged from the parent |
...
Phases 0-9 of `doitall.sh` are also identical. The current 22a and 22b
mixing-period vectors are equal, but their source-method provenance remains
separate. The final script checks that exactly one tau is estimated and writes
`tag-tau-audit.csv`.
