...
# -------------------------------------------------------
#  PHASE 10 - estimate one common tag-recapture tau
# -------------------------------------------------------

# The direct negative-binomial parameterization is
# tau = 1 + exp(fish_pars(4)). The native transformed bounds are retained:
# fish_pars(4) is bounded from -5 to 5, so tau starts at 2 and has an
# effective lower bound of 1 + exp(-5) = 1.006737947.
...
tag_tau_group_controls=$(
  ...
  while [ "$fishery" -le 33 ]; do
    if [ "$fishery" -le 28 ]; then
      # F1-F28 are the capture fisheries and share one common tau.
      printf '  -%s 43 1 -%s 44 1\n' "$fishery" "$fishery"
    else
      # F29-F33 are abundance-index fisheries with no tag recaptures.
...
echo "TAG tau grouping: common F1-F28 ($expected_tau_count estimated parameter)"
echo "TAG tau parameterization: tau = 1 + exp(fish_pars(4))"
echo "TAG tau native bounds: theta [-5, 5]; effective tau lower 1.006737947"
echo "TAG tau starting value: 2"

# Phases 0-9 remain identical to the corresponding no-tau baseline. Reset only
# fishery-parameter row 4 before opening the single common tau in Phase 10.
...
$program_path bet.frq 09.tau.par 10.par -file - <<PHASE10
  1 111 4    # recommended negative-binomial tag-recapture likelihood
  1 177 0    # retain the full tag-likelihood weight
  ...
  1 306 0    # retain the native transformed bounds (-5, 5)
  1 358 0    # retain the native upper-bound rule
  2 100 0    # standard tag-return calculation
  2 121 0    # do not use the tag survival-analysis likelihood
  2 122 0    # keep tag-recapture data active
$tag_tau_group_controls
  1 1 10000  # retain the baseline Phase 10 evaluation limit
  1 50 $phase10_11_convergence
  1 121 0    # keep the fixed Lorenzen M used by the baseline
PHASE10
...
$program_path bet.frq 10.par 11.par -file - <<PHASE11
  1 1 5000   # retain the baseline Phase 11 evaluation limit
  1 50 $phase10_11_convergence
  1 121 0    # keep the fixed Lorenzen M used by the baseline
...
# Validate that exactly one direct tau is estimated and that no non-tag
# settings needed for this sensitivity changed.
actual_tau_count=$(awk '$2 ~ /^fish_pars[(]4[)]/ {n++} END {print n+0}' indepvar.rpt)
if [ "$actual_tau_count" -ne "$expected_tau_count" ]; then
  echo "Final fit estimated $actual_tau_count tau parameters; expected $expected_tau_count." >&2
  exit 44
fi
...
if [ "$parest_111" != 4 ] || [ "$parest_121" != 0 ] ||
   ...
   [ "$parest_306" != 0 ] || [ "$parest_342" != 25 ] ||
   [ "$parest_358" != 0 ]; then
  echo "Final parameter file did not retain the required common-tau, fixed-M, full-tag-weight, or Nmax25 controls." >&2
  exit 47
fi

printf '%s\n' \
  'grouping,estimated_tau_count,tau_start,effective_tau_lower,parest111,parest121,parest177,parest239,parest249,parest305,parest306,parest342,parest358,status' \
  "common-F1-F28,$actual_tau_count,2,1.006737947,$parest_111,$parest_121,$parest_177,$parest_239,$parest_249,$parest_305,$parest_306,$parest_342,$parest_358,passed" \
  > tag-tau-audit.csv
