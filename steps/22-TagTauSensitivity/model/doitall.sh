...
if [ ! -f ./tag_tau_scenarios.sh ]; then
  echo "tag_tau_scenarios.sh is missing from the model folder." >&2
  exit 1
fi
...
tag_tau_scenario=${TAG_TAU_SCENARIO:-g01-common}
if ! tag_tau_scenario_is_supported "$tag_tau_scenario"; then
  echo "Unsupported TAG_TAU_SCENARIO: $tag_tau_scenario" >&2
  exit 1
fi
if ! tag_tau_validate_mapping "$tag_tau_scenario"; then
  echo "Invalid or unsupported TAG tau group map: $tag_tau_scenario" >&2
  exit 1
fi
...
tag_tau_group_controls=$(tag_tau_write_controls "$tag_tau_scenario")
echo "TAG tau scenario: $tag_tau_scenario"
echo "TAG tau groups: $(tag_tau_expected_groups "$tag_tau_scenario")"
echo "TAG tau lower bound: $((tag_tau_lower_x100 / 100))"
echo "TAG tau starting value: $((tag_tau_lower_x100 / 100 + 1))"
...
# Set fishery-parameter row 4 only in the Phase 10 input par. Phases 0-9
# remain unchanged, and direct tau starts one unit above its selected lower
# bound (tau = 3, 4, or 5).
...
$program_path bet.frq 09.tau.par 10.par -file - <<PHASE10
  ...
  1 306 $tag_tau_lower_x100  # lower tau bound in hundredths; upper bound is 50 times lower
...
$tag_tau_group_controls
  1 1 3000   # stabilize the newly opened tau parameter(s)
  1 50 -3
...
$program_path bet.frq 10.par 11.par -file - <<PHASE11
  1 1 10000
  1 50 $phase11_12_convergence  # final MGC target; default 1e-4
  1 121 0    # keep Lorenzen natural mortality fixed
PHASE11
