...
| Field | Value |
| --- | --- |
| Step folder | `steps/04-LengthWeight/model` |
...
| # | Change |
| --- | --- |
| 1 | Inherits the diagnostic-side 2023 assessment replication model from `03-FixM`. |
...
| File | Source / note |
| --- | --- |
| `.frq` | `steps/03-FixM/model/bet.frq` |
| `.ini` | `steps/03-FixM/model/bet.ini`; set Length-weight parameters from `3.063397e-05 2.932384` to `3.073533e-05 2.932410`; Lorenzen natural-mortality scaling fixed to the 2023 diagnostic-model estimate from the 01-Diag2023 mgc=-5 diagnostic final par |
| `.tag` | `steps/03-FixM/model/bet.tag` |
| `.age_length` | `steps/03-FixM/model/bet.age_length` |
...
| --- | --- | --- |
| `.ini` | Changes only `# Length-weight parameters` to `3.073533e-05 2.932410`. | All other `03-FixM` ini controls. |
| `.frq/.tag/.age_length` | No generated edit. | Inherited from `03-FixM`. |
...
| # | Check |
| --- | --- |
| 1 | Compare directly with 03-FixM to isolate this substep's change. |
