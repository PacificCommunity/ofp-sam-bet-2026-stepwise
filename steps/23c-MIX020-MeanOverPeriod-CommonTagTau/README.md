# 23c: K=0.2 mean over period with one common tag tau

This sensitivity is an exact clone of `22a-MIX020-MeanOverPeriod` except for
the tag-recapture tau controls added to `doitall.sh` from Phase 10 onward.

| Item | Setting |
| --- | --- |
| Scientific parent | `22a-MIX020-MeanOverPeriod` |
| Mixing periods | Main-branch K=0.2 mean over period; unchanged from the parent |
