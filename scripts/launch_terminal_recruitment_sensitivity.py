...
95 merged model bundles then feed the established BET Kflow results task,
...
95-model grid into 30 Hessian shards per model.
...
Examples:
  ...
  python3 scripts/launch_terminal_recruitment_sensitivity.py \
    --models 12-OrthogonalPoly,12d-OPR69AllEnd3 --hessian-nsplit auto
"""
...
DEFAULT_STEPWISE_TASK = "ofp-sam-bet-2026-stepwise-terminal-recruitment"
...
def fit_payload(row: dict[str, str], args: argparse.Namespace, branch: str) -> dict[str, Any]:
    step = row["step_id"]
    title = row["job_title"] or f"Terminal sensitivity {step}"
    description = row["change_axis"] or f"Terminal-recruitment sensitivity {step}."
    ...
    env = {
        ...
        "JOB_KEY": row["job_key"] or step.lower(),
    ...
    return {
        **submitter_fields(args),
        ...
        "tags": {
            ...
            "experiment": "terminal-recruitment",
            "model": step,
...
def hessian_payload(row: dict[str, str], args: argparse.Namespace, branch: str, fit_job: str, part: int) -> dict[str, Any]:
    title = f"Hessian {part}/{args.hessian_nsplit}: {row['step_id']}"
    description = f"One-part-per-model Hessian diagnostic for terminal sensitivity {row['step_id']}."
...
def hessian_merge_payload(row: dict[str, str], args: argparse.Namespace, branch: str, hessian_jobs: list[str]) -> dict[str, Any]:
    title = f"Hessian merge: {row['step_id']}"
    description = f"Merge Hessian output for terminal sensitivity {row['step_id']}."
...
def results_payload(
    ...
    input_jobs = list(dict.fromkeys(merge_jobs))
    title = f"BET terminal-recruitment sensitivity results ({len(models)} models)"
    description = (
    ...
    env = {
        ...
        "JOB_TITLE": title,
        "JOB_DESCRIPTION": description,
        "JOB_KEY": "terminal-recruitment-sensitivity-results",
        "PLOT_TITLE": "BET 2026 terminal-recruitment sensitivity results",
        "MFCLSHINY_INTERACTIVE_VIEWER_TITLE": "BET 2026 terminal-recruitment sensitivity viewer",
...
def load_or_create_manifest(path: Path, args: argparse.Namespace, branch: str, models: list[dict[str, str]]) -> dict[str, Any]:
    if args.resume and path.exists():
        ...
        return manifest
    return {
        "schema": "bet-2026-terminal-recruitment-launch-v1",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--branch", default="", help="Source branch; defaults to the checked-out branch.")
    ...
    parser.add_argument("--flow-group", default=f"bet-2026-terminal-recruitment-{date_label}")
...
def main() -> int:
    ...
    input_refs = [ref for ref in merge_refs if ref]
    if input_refs and not job_number(manifest, "results"):
        try:
        ...
        errors.append("results: no Hessian-merge job references were created")
