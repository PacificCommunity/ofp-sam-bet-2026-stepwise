...
def build_payload(
    ...
    # Kflow merges incoming metadata with the existing report metadata when the
    # repository is unchanged. Send an explicit empty list when a config has no
    # local apps so stale button definitions from older registrations are cleared.
