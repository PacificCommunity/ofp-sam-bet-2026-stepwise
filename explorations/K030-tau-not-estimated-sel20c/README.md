# K030 — tau not estimated — 20c selectivity

Self-contained Job 17805-control exploration using the Joe all-region-mean
mixing-period vector at K=0.30 with tau not estimated. Run it through the
repository-level `run.sh` with `MODEL=K030-tau-not-estimated-sel20c`.
