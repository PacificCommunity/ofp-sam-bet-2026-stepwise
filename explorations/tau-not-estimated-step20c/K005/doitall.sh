...
# ------------------------------------------------------------------
#  STEP 20c TAG-TAU TREATMENT - negative binomial, tau not estimated
# ------------------------------------------------------------------

# This branch reproduces the Step 20c treatment: parest flag 111 remains 4
# (negative binomial), while fish flags 43/44 remain zero and fish_pars(4)
# is not opened as an independent variable. All Job 17805 controls applied
# above, including the F14/F15 and selectivity changes, remain unchanged.
...
if [ "$parest_111" != 4 ] || [ "$parest_121" != 0 ] ||
   [ "$parest_342" != "$dm_nmax_flag" ] ||
   [ "$estimated_tau_count" != 0 ] || [ "$active_tau_fisheries" != 0 ]; then
  echo "Final fit did not retain the Step 20c negative-binomial, tau-not-estimated controls." >&2
  exit 44
fi
...
printf '%s\n' \
  'mode,tag_likelihood,parest111,estimated_tau_count,active_tau_fisheries,parest121,parest342,final_m,status' \
  "step20c-not-estimated,negative-binomial,$parest_111,$estimated_tau_count,$active_tau_fisheries,$parest_121,$parest_342,$final_m,passed" \
  > tag-tau-audit.csv
exit 0
