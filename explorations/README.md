...
| Model suffix | Tag likelihood and tau treatment |
| --- | --- |
| `K*-tau-estimated` | Negative binomial; one common F1-F28 tau is estimated |
| `K*-tau-not-estimated` | Negative binomial; tau is not estimated |
...
movement penalties are 0.1; and OPR is off. The two modes differ only in tau
estimation. The not-estimated mode does not turn the tag likelihood into
Poisson: it retains negative-binomial flag 111=4, leaves fish flags 43/44
inactive, and estimates zero `fish_pars(4)` parameters.
